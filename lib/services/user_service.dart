...
class UserService {
  ...
  Future<bool> register(UserModel user, String password, String confirmPassword) async {
    try {
      ...
      // ✅ FIX: saveUser بترجع early لو currentUser == null && token == null
      // فبنحفظ المستخدم مباشرةً هنا بدون ما نعتمد على الـ guard
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('user_data', jsonEncode(user.toJson()));
      if (token != null) {
        await prefs.setString('auth_token', token);
      ...
      currentUser.value = user; // ← المهم: currentUser.value يتعين
